...
<head>
    ...
    <style>
        ...
        .ringkasan-table .col-ringkasan-no {
            width: 3%;
        }

        .ringkasan-table .col-ringkasan-nama {
            width: 20%;
        }

        .ringkasan-table .col-ringkasan-vol {
            width: 6%;
        }

        .ringkasan-table .col-ringkasan-total {
            width: 9%;
        }

        .ringkasan-grandtotal {
...
<div class="ringkasan-section">
    ...
    <table class="ringkasan-table">
        <thead>
            <tr>
                <th rowspan="2" class="col-ringkasan-no">No</th>
                <th rowspan="2" class="col-ringkasan-nama">RINGKASAN</th>
                
                <th colspan="2">SALDO AKHIR<br>{{ $tanggalSaldoAkhir }}</th>
                <th colspan="2">PENGADAAN<br>s/d {{ $tanggalAkhir }}</th>
                <th colspan="2">JUMLAH SAMPAI<br>s/d {{ $tanggalAkhir }}</th>
                <th colspan="2">JUMLAH PENGGUNAAN<br>s/d {{ $tanggalAkhir }}</th>
                <th colspan="2">SISA PER<br>{{ $tanggalSisaPer }}</th>
            </tr>
            <tr>
                <th class="col-ringkasan-vol">Vol</th>
                <th class="col-ringkasan-total">Total</th>
        ...
        <tbody>
            @foreach ($summaryData as $index => $summary)
                <tr>
                    ...
                    <td class="text-left">{{ strtoupper($summary['name']) }}</td>
                    
                    <td class="text-right number">{{ number_format($summary['saldo_akhir_vol'], 0, ',', '.') }}</td>
                    <td class="text-right number">{{ formatNumber($summary['saldo_akhir_total']) }}</td>
                    
                    <td class="text-right number">{{ number_format($summary['pengadaan_vol'], 0, ',', '.') }}</td>
                    <td class="text-right number">{{ formatNumber($summary['pengadaan_total']) }}</td>
                    
                    <td class="text-right number">{{ number_format($summary['jumlah_sampai_vol'], 0, ',', '.') }}</td>
                    <td class="text-right number">{{ formatNumber($summary['jumlah_sampai_total']) }}</td>
                    
                    <td class="text-right number">{{ number_format($summary['jumlah_penggunaan_vol'], 0, ',', '.') }}</td>
                    <td class="text-right number">{{ formatNumber($summary['jumlah_penggunaan_total']) }}</td>
                    
                    <td class="text-right number">{{ number_format($summary['sisa_per_vol'], 0, ',', '.') }}</td>
                    <td class="text-right number">{{ formatNumber($summary['sisa_per_total']) }}</td>
                </tr>
            @endforeach

            {{-- GRAND TOTAL --}}
            <tr class="ringkasan-grandtotal">
                <td colspan="2" class="text-center">JUMLAH</td>
                
                <td class="text-right number">{{ number_format($grandTotal['saldo_akhir_vol'], 0, ',', '.') }}</td>
                <td class="text-right number">{{ formatNumber($grandTotal['saldo_akhir_total']) }}</td>
                
                <td class="text-right number">{{ number_format($grandTotal['pengadaan_vol'], 0, ',', '.') }}</td>
                <td class="text-right number">{{ formatNumber($grandTotal['pengadaan_total']) }}</td>
                
                <td class="text-right number">{{ number_format($grandTotal['jumlah_sampai_vol'], 0, ',', '.') }}</td>
                <td class="text-right number">{{ formatNumber($grandTotal['jumlah_sampai_total']) }}</td>
                
                <td class="text-right number">{{ number_format($grandTotal['jumlah_penggunaan_vol'], 0, ',', '.') }}</td>
                <td class="text-right number">{{ formatNumber($grandTotal['jumlah_penggunaan_total']) }}</td>
                
                <td class="text-right number">{{ number_format($grandTotal['sisa_per_vol'], 0, ',', '.') }}</td>
                <td class="text-right number">{{ formatNumber($grandTotal['sisa_per_total']) }}</td>
            </tr>
        </tbody>
    </table>
</div>
